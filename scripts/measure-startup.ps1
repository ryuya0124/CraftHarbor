...
for ($i = 0; $i -lt $Runs; $i++) {
    ...
    try {
        while ($watch.Elapsed.TotalSeconds -lt 20 -and -not $process.HasExited) {
            ...
            if ($process.MainWindowTitle -like '*Minecraft Server Manager*' -or $process.MainWindowTitle -like '*Minecraft Server Control*' -or $process.MainWindowTitle -like '*マインクラフト サーバー管理*') { $ready = $watch.ElapsedMilliseconds; break }
