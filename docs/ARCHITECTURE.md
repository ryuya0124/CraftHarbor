...
プリセットは全体バックアップ後に3フォルダを切り替え、例外時は全体バックアップから復元します。プロセス停止中の操作だけをUIから許可します。MOD群は全ファイルを取得してから移動し、途中の移動失敗時は移動済み分を戻します。
...
MainWindowはコード生成の9画面と共通スタイルで構成しています。初版の依存関係を減らすため外部MVVMライブラリは使用していません。ページ単位のViewModelへの分割、ジョブ単位の操作ロック、ジョブ復旧は今後の拡張項目です。
